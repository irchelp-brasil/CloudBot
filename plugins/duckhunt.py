...
def start_hunt(db, chan, message, conn):
    ...
    if not chan.startswith("#"):
        return "Não casse sozinho. Não é seguro."
    ...
    message(
        ...
        "Vamos ver quantos você mata ou salva. "
        "Use .bang para matar, trucidar os patos ou .befriend para salva-los. "
        "NOTE: Os patos só voam se houver alguma atividade no canal. Do contratio, eles ficam entocados.",
        chan
    )
...
def stop_hunt(db, chan, conn):
    ...
    if get_state_table(conn.name, chan).game_on:
        ...
        return "O jogo esta interrompido.."

    return "Não há nenhum jogo no {}.".format(chan)
...
def attack(event, nick, chan, db, conn, attack_type):
    ...
    if attack_type == "shoot":
        ...
        msg = "{} Você matou o pato em {:.3f} segundos! Tem o total de {} mortes no {}."
        scripter_msg = "Você puxou o gatinho em {:.3f} segundos, isso é muito rapido Billy The Kid. " \
                       "Tens certeza que isso não é um script? Vai ficar por 2 horas na geladeira."
        attack_type = "shoot"
    else:
        miss = [
        ...
        msg = "{} Você conquistou a amizade do pato em {:.3f} seconds! Grande Dom Juan! Você já conquistou {} no {}."
...
def top_list(prefix, data, join_char=' â€¢ '):
    r"""
    >>> foods = [('Spam', 1), ('Eggs', 4)]
    >>> top_list("Top Foods: ", foods)
    'Top Foods: \x02E\u200bggs\x02: 4 â€¢ \x02S\u200bpam\x02: 1'
    """
...
def display_scores(score_type: ScoreType, event, text, chan, conn, db):
    ...
    no_ducks = "Parece que ninguém {verb} nenhum pato ainda."
...
def friends(text, event, chan, conn, db):
    """[{global|average}] - Mostra a lista dos usuários que mais salvaram patos no canal.
    Se o parâmetro 'global' é especificado, todos os canais da base de dados são incluídos.
...
def killers(text, event, chan, conn, db):
    """[{global|average}] - Mostra a lista dos usuários que mais mataram patos no canal.
    Se o parâmetro 'global' é especificado, todos os canais da base de sados são incluídos.
...
def duck_stats(chan, conn, db, message):
    """- Informa o scode do canal e o score da Rede, mostrando o ranking deste ultimo.
    ...
    if scores:
        ...
        message(
            "\x02Duck Stats:\x02 {:,} mortes e {:,} salvos no \x02{}\x02. "
